    ...
    def test(self):
        ...
        for k in range(image_num):
            with torch.no_grad():
                ...
                x1_pad = torch.zeros(1, C, h + pad // 2 + edge_H // 4, w + pad // 2 + edge_W // 4).cuda(self.gpu_ids[0]) # 128 + 4//2 + 64//4 = 146  (1 , 8 , 146, 146)
                x2_pad = torch.zeros(1, 1, H + pad * 2 + edge_H, W + pad * 2 + edge_W).cuda(self.gpu_ids[0])  # 512 + 4*2 + 64 = 584 (1 , 1, 584 ,584)
